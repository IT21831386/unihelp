...
function Dashboard() {
  ...
  const handleUpdateMarketplaceItem = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`http://localhost:5000/api/marketplace/${editingMarketplaceItem._id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(editingMarketplaceItem)
      });
      if (res.ok) {
        const updated = await res.json();
        // The API returns { item: ... } based on my observation of marketplaceController.js
        const updatedItem = updated.item || updated;
        setMarketplaceItems(prev => prev.map(item => item._id === updatedItem._id ? updatedItem : item));
        ...
        return (
          <div className="dashboard-card">
            <div className="dashboard-card__header">
              ...
              <span className="dashboard-badge">{marketplaceItems.length} Items</span>
            </div>
            <div className="table-responsive">
              <table className="dashboard-table">
                <thead>
                ...
                <tbody>
                  {marketplaceItems.length === 0 ? (
                    <tr><td colSpan="7" className="empty-row">No marketplace items found</td></tr>
                  ) : (
                    marketplaceItems.map(item => (
                      <tr key={item._id}>
                        ...
                        <td>
                          <div style={{ display: 'flex', gap: '8px' }}>
                            <button onClick={() => setViewingMarketplaceItem(item)} style={{ padding: '5px 10px', background: '#e1e4e8', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold' }}>View</button>
                            <button onClick={() => setEditingMarketplaceItem({ ...item })} style={{ padding: '5px 10px', background: '#0366d6', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold' }}>Edit</button>
                            <button onClick={() => handleDeleteMarketplaceItem(item._id)} style={{ padding: '5px 10px', background: '#dc3545', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold' }}>Delete</button>
                          </div>
                        </td>
                      </tr>
                    ))
            ...
            {viewingMarketplaceItem && (
              <div onClick={() => setViewingMarketplaceItem(null)} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
                <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: '12px', padding: '30px', maxWidth: '500px', width: '90%', textAlign: 'left', boxShadow: '0 8px 32px rgba(0,0,0,0.2)' }}>
                  <h3 style={{ marginBottom: '15px' }}>{viewingMarketplaceItem.itemName}</h3>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '20px' }}>
                    <div><strong>Category:</strong> {viewingMarketplaceItem.category}</div>
                    <div><strong>Price:</strong> LKR {viewingMarketplaceItem.price.toLocaleString()}</div>
                    <div><strong>Condition:</strong> {viewingMarketplaceItem.condition}</div>
                    <div><strong>Status:</strong> {viewingMarketplaceItem.status}</div>
                    <div><strong>Seller:</strong> {viewingMarketplaceItem.sellerName}</div>
                    <div><strong>Negotiable:</strong> {viewingMarketplaceItem.isNegotiable ? 'Yes' : 'No'}</div>
                  </div>
                  <div style={{ marginBottom: '20px' }}>
                    <strong>Description:</strong>
                    <p style={{ marginTop: '5px', color: '#586069' }}>{viewingMarketplaceItem.description}</p>
                  </div>
                  <button onClick={() => setViewingMarketplaceItem(null)} style={{ width: '100%', padding: '10px', background: '#e1e4e8', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' }}>Close</button>
                </div>
              </div>
            )}
            ...
            {editingMarketplaceItem && (
              <div onClick={() => setEditingMarketplaceItem(null)} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
                <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: '12px', padding: '30px', maxWidth: '500px', width: '90%', boxShadow: '0 8px 32px rgba(0,0,0,0.2)', maxHeight: '90vh', overflowY: 'auto' }}>
                  <h3 style={{ marginBottom: '20px' }}>Edit Marketplace Item</h3>
                  <form onSubmit={handleUpdateMarketplaceItem} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                    <div>
                      <label style={{ display: 'block', fontSize: '14px', fontWeight: 'bold', marginBottom: '5px' }}>Item Name</label>
                      <input type="text" value={editingMarketplaceItem.itemName} onChange={e => setEditingMarketplaceItem({ ...editingMarketplaceItem, itemName: e.target.value })} style={{ width: '100%', padding: '8px', border: '1px solid #d1d5da', borderRadius: '4px' }} required />
                    </div>
                    <div style={{ display: 'flex', gap: '15px' }}>
                      <div style={{ flex: 1 }}>
                        <label style={{ display: 'block', fontSize: '14px', fontWeight: 'bold', marginBottom: '5px' }}>Category</label>
                        <select value={editingMarketplaceItem.category} onChange={e => setEditingMarketplaceItem({ ...editingMarketplaceItem, category: e.target.value })} style={{ width: '100%', padding: '8px', border: '1px solid #d1d5da', borderRadius: '4px' }}>
                          {['Electronics', 'Furniture', 'Books', 'Clothing', 'Boarding', 'Sports', 'Stationery', 'Other'].map(cat => <option key={cat} value={cat}>{cat}</option>)}
                        </select>
                      </div>
                      <div style={{ flex: 1 }}>
                        <label style={{ display: 'block', fontSize: '14px', fontWeight: 'bold', marginBottom: '5px' }}>Price (LKR)</label>
                        <input type="number" value={editingMarketplaceItem.price} onChange={e => setEditingMarketplaceItem({ ...editingMarketplaceItem, price: Number(e.target.value) })} style={{ width: '100%', padding: '8px', border: '1px solid #d1d5da', borderRadius: '4px' }} required />
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: '15px' }}>
                      <div style={{ flex: 1 }}>
                        <label style={{ display: 'block', fontSize: '14px', fontWeight: 'bold', marginBottom: '5px' }}>Condition</label>
                        <select value={editingMarketplaceItem.condition} onChange={e => setEditingMarketplaceItem({ ...editingMarketplaceItem, condition: e.target.value })} style={{ width: '100%', padding: '8px', border: '1px solid #d1d5da', borderRadius: '4px' }}>
                          {['Like New', 'Good', 'Fair', 'Poor'].map(cond => <option key={cond} value={cond}>{cond}</option>)}
                        </select>
                      </div>
                      <div style={{ flex: 1 }}>
                        <label style={{ display: 'block', fontSize: '14px', fontWeight: 'bold', marginBottom: '5px' }}>Status</label>
                        <select value={editingMarketplaceItem.status} onChange={e => setEditingMarketplaceItem({ ...editingMarketplaceItem, status: e.target.value })} style={{ width: '100%', padding: '8px', border: '1px solid #d1d5da', borderRadius: '4px' }}>
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '14px', fontWeight: 'bold', marginBottom: '5px' }}>Description</label>
                      <textarea value={editingMarketplaceItem.description} onChange={e => setEditingMarketplaceItem({ ...editingMarketplaceItem, description: e.target.value })} style={{ width: '100%', padding: '8px', border: '1px solid #d1d5da', borderRadius: '4px' }} rows={4} required />
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px' }}>
                      <button type="button" onClick={() => setEditingMarketplaceItem(null)} style={{ padding: '8px 15px', background: '#e1e4e8', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>Cancel</button>
                      <button type="submit" style={{ padding: '8px 15px', background: 'var(--color-primary)', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>Save Changes</button>
                    </div>
                  </form>
                </div>
                              ...
                              onClick={async () => {
                                ...
                                try {
                                  const res = await fetch('http://localhost:5000/api/marketplace/unsave', {
                                    method: 'POST',
                                    headers: { 'Content-Type': 'application/json' },
                                    body: JSON.stringify({ userId: currentUser.id || currentUser._id, itemId: item._id })
                                  });
                                  if (res.ok) {
                                    setSavedMarketplaceItems(prev => prev.filter(i => i._id !== item._id));
                                  }
                                } catch (e) { alert('Error updating saved items'); }
                              }}
